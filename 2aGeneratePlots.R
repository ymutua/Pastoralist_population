...
max_val_0 <- max(c(agroPoly_0$wrldPp_, pastoralPoly_0$wrldPp_), na.rm = TRUE)
max_val_1 <- max(c(agroPoly_1$wrldPp_, pastoralPoly_1$wrldPp_), na.rm = TRUE)

# Country level (0) plots
p1a <- ggplot(agroPoly_0) +
  geom_sf(aes(fill = wrldPp_), color = "black", size = 0.1) +
